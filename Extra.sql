...
--1. Departments with at least 3 employees where the average salary is greater than 60,000
SELECT DEPARTMENT,COUNT(EmpID) FROM Employee
GROUP BY DEPARTMENT
HAVING AVG(Salary)>60000 AND COUNT(EmpID)>3;
--2. Show each city where the highest salary is greater than 90,000 and the lowest salary is less
--than 30,000
SELECT CITY FROM Employee
GROUP BY CITY
HAVING MAX(Salary)>90000;
--3. Find departments where the difference between the highest and lowest salary is more than
--50,000
SELECT DEPARTMENT FROM Employee
GROUP BY DEPARTMENT
HAVING (MAX(SALARY)-MIN(SALARY))>50000
--4. Show department and designaƟon combinaƟons having more than 2 employees and total
--salary greater than 2,00,000
SELECT department FROM Employee
--5. Show departments where the average salary is between 50,000 and 80,000 and the
--maximum salary is at least twice the minimum salary
--6. Show departments where all employees earn more than ₹40,000.
--7. Show ciƟes having employees from at least 3 different departments.
--8. Show departments where the total salary is greater than ₹5,00,000 and there are employees
--of both genders.
--9. Show joining years where more than 5 employees joined and the average salary is less than
--₹55,000.
--10. Show department and city combinaƟons where:
--At least 4 employees work,
--The average salary is above ₹60,000,
--The highest salary is below ₹1,20,000.
--11. Find departments where all employees joined in or aŌer 2020.
--12. Show ciƟes where every employee has a unique salary (no duplicate salary within the city)